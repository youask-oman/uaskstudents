...
const AudioUtil = __importStar(require("./audio_util.js"));
...
LayoutRenderer.options = {
    cayleyshort: engine_js_1.Engine.getInstance().options.cayleyshort,
    linebreaks: engine_js_1.Engine.getInstance().options.linebreaks
};
let twodExpr = '';
const handlers = {
    TABLE: handleTable,
...
function setTwoDim(str) {
    ...
    debugger_js_1.Debugger.getInstance().generate(() => [DomUtil.formatXml(dom.toString())]);
...
function handleCayley(cayley) {
    ...
    if (engine_js_1.Engine.getInstance().options.cayleyshort && mat[0].cells[0] === '⠀') {
        bar.cells[0] = '⠀';
    }
    mat.splice(1, 0, bar);
    mat = combineCells(mat, maxWidth);
...
function handleRelation(rel) {
    if (!engine_js_1.Engine.getInstance().options.linebreaks) {
...
//# sourceMappingURL=layout_renderer.js.map