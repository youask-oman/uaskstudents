...
function setSubiso(msg) {
    const subiso = engine_js_1.Engine.getInstance().options.subiso;
    if (msg.SUBISO.all.indexOf(subiso) === -1) {
        engine_js_1.Engine.getInstance().options.subiso = msg.SUBISO.default;
    }
    msg.SUBISO.current = engine_js_1.Engine.getInstance().options.subiso;
}
function getLocale() {
    const locale = variables_js_1.Variables.ensureLocale(engine_js_1.Engine.getInstance().options.locale, engine_js_1.Engine.getInstance().defaultLocale);
    engine_js_1.Engine.getInstance().options.locale = locale;
