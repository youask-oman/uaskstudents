...
var TestMmlVisitor = (function (_super) {
    ...
    TestMmlVisitor.prototype.visitDefault = function (node, space) {
        ...
        var attributes = this.attributeString({
            isEmbellished: node.isEmbellished,
            isSpacelike: node.isSpacelike,
            texClass: node.texClass,
        }, '{', '}');
        var mml = "".concat(space, "<").concat(kind).concat(this.getAttributes(node)).concat(this.getInherited(node)).concat(this.getProperties(node), "\n") +
            "".concat(space, "   ").concat(attributes, ">").concat(nl);
        space += '  ';
        try {
        ...
        mml += "".concat(endspace, "</").concat(kind, ">");
        return mml;
    };
    ...
    TestMmlVisitor.prototype.attributeString = function (attributes, open, close) {
        ...
        try {
            for (var _b = __values(Object.keys(attributes)), _c = _b.next(); !_c.done; _c = _b.next()) {
                var name_1 = _c.value;
                ATTR += " ".concat(open).concat(name_1, "=\"").concat(this.quoteHTML(String(attributes[name_1])), "\"").concat(close);
            }
        }
