...
exports.TextMacrosMethods = {
            ...
            switch (c) {
                ...
                case '$':
                    if (braces === 0 && open === c) {
                        ...
                        var mml = new TexParser_js_1.default(parser.string.substring(i, j), parser.stack.env, config).mml();
                        parser.PushMath(mml);
                        return;
                    }
                    break;
    ...
    TextFont: function (parser, name, variant) {
        parser.saveText();
        parser.Push(parser.ParseTextArg(name, { mathvariant: variant }));
    },
    ...
    CheckAutoload: function (parser, name) {
        ...
        if (!macro || (autoload && macro._func === autoload.Autoload)) {
            texParser.parse(HandlerTypes_js_1.HandlerType.MACRO, [texParser, name]);
            if (!macro)
                return;
    ...
    UnderOver: BaseMethods_js_1.default.UnderOver,
    Lap: BaseMethods_js_1.default.Lap,
