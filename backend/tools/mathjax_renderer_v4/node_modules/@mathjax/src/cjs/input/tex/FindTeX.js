    ...
    FindTeX.prototype.findEnd = function (text, n, start, end) {
        ...
        while ((match = pattern.exec(text))) {
            if ((match[1] || match[0]) === close && braces === 0) {
                return (0, MathItem_js_1.protoItem)(start[0], text.substring(i, match.index), match[0], n, start.index, match.index + match[0].length, display);
            }
            else if (match[0] === '{') {
                braces++;
            }
    ...
    FindTeX.prototype.findMathInString = function (math, n, text) {
        ...
        while ((start = this.start.exec(text))) {
            if (start[this.env] !== undefined && this.env) {
                ...
                if (math_1.length === 2) {
                    match = (0, MathItem_js_1.protoItem)('\\', math_1.substring(1), '', n, start.index, end);
                }
                else {
