...
exports.SetOptionsUtil = {
    ...
    filterOption: function (parser, extension, option) {
        var config = parser.options.setoptions;
        var options = config.allowOptions[extension] || {};
        var isTex = extension === 'tex';
        var allow = Object.hasOwn(options, option) && !(0, Options_js_1.isObject)(options[option])
            ? options[option]
            : null;
        if (allow === false || (allow === null && !config.allowOptionsDefault)) {
            if (isTex) {
                throw new TexError_js_1.default('TeXOptionNotSettable', 'Option "%1" is not allowed to be set', option);
            }
            else {
                throw new TexError_js_1.default('OptionNotSettable', 'Option "%1" is not allowed to be set for package %2', option, extension);
            }
        }
        var extOptions = isTex ? parser.options : parser.options[extension];
    ...
    filterValue: function (_parser, _extension, _option, value) {
        ...
        var match = value.match(/^\/(.*)\/([dgimsuvy]*)$/);
...
function SetOptions(parser, name) {
    ...
    var options = ParseUtil_js_1.ParseUtil.keyvalOptions(parser.GetArgument(name));
    var config = parser.options.setoptions;
...
function setoptionsConfig(_config, jax) {
    var setOptionsMap = new TokenMap_js_1.CommandMap('setoptions', {
        setOptions: SetOptions,
    });
    var macros = jax.parseOptions.handlers.get(HandlerTypes_js_1.HandlerType.MACRO);
    macros.add(['setoptions'], null, PrioritizedList_js_1.PrioritizedList.DEFAULTPRIORITY - 1);
    var require = macros.lookup('require');
    if (require) {
        setOptionsMap.add('Require', new Token_js_1.Macro('Require', require._func));
        setOptionsMap.add('require', new Token_js_1.Macro('require', BaseMethods_js_1.default.Macro, [
...
exports.SetOptionsConfiguration = Configuration_js_1.Configuration.create('setoptions', (_a = {},
    _a[HandlerTypes_js_1.ConfigurationType.CONFIG] = setoptionsConfig,
