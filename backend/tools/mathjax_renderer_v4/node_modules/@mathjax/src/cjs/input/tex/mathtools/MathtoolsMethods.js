...
exports.MathtoolsMethods = exports.LEGACYPRIORITY = exports.LEGACYCONFIG = void 0;
...
exports.LEGACYCONFIG = (_a = {},
    _a[HandlerTypes_js_1.HandlerType.MACRO] = ['mathtools-legacycolonsymbols'],
    _a);
exports.LEGACYPRIORITY = PrioritizedList_js_1.PrioritizedList.DEFAULTPRIORITY - 1;
exports.MathtoolsMethods = {
    ...
    MtSmallMatrix: function (parser, begin, open, close, align) {
        ...
        return exports.MathtoolsMethods.Array(parser, begin, open, close, align, UnitUtil_js_1.UnitUtil.em(1 / 3), '.2em', 'S', 1);
    },
    MtMultlined: function (parser, begin) {
        var name = "\\begin{".concat(begin.getName(), "}");
        var pos = parser.options.mathtools['multlined-pos'] || 'c';
        var width = parser.options.mathtools['multlined-width'] || '';
        if (!parser.nextIsSpace()) {
            var arg = parser.GetBrackets(name, pos);
            if (arg.match(/^[ctb]$/)) {
                pos = arg;
            ...
            if (width && !UnitUtil_js_1.UnitUtil.matchDimen(width)[0]) {
        ...
        return ParseUtil_js_1.ParseUtil.setArrayAlign(item, pos);
    },
    ...
    MathLap: function (parser, name, pos, cramped) {
        ...
        var mml = parser.create('node', 'mstyle', [
            parser.create('node', 'mpadded', [parser.ParseArg(name)], __assign({ width: 0 }, (pos === 'r'
                ? {}
    ...
    UnderOverBracket: function (parser, name) {
        ...
        var node = ParseUtil_js_1.ParseUtil.underOver(parser, base, script, pos, true);
    ...
    Aboxed: function (parser, name, box, math) {
        if (box === void 0) { box = 'boxed'; }
        if (math === void 0) { math = true; }
        ...
        var arg = parser.GetArgument(name);
        var rest = parser.string.substring(parser.i);
        parser.string = arg + '&&\\endAboxed';
        parser.i = 0;
        var left = parser.GetUpTo(name, '&');
        var right = parser.GetUpTo(name, '&');
        parser.GetUpTo(name, '\\endAboxed');
        var _a = __read(math ? ['', ''] : ['$\\displaystyle{', '}$'], 2), bmath = _a[0], emath = _a[1];
        var tex = ParseUtil_js_1.ParseUtil.substituteArgs(parser, [left, right], "\\rlap{\\".concat(box, "{").concat(bmath, "#1{}#2").concat(emath, "}}") +
            '\\kern.267em\\phantom{#1}&\\phantom{{}#2}\\kern.267em');
        parser.string = tex + rest;
        parser.i = 0;
    },
    MakeAboxedCommand: function (parser, name) {
        var star = parser.GetStar();
        var cs = NewcommandUtil_js_1.NewcommandUtil.GetCSname(parser, name);
        var box = NewcommandUtil_js_1.NewcommandUtil.GetCSname(parser, name + '\\' + cs);
        var handlers = parser.configuration.handlers;
        if (handlers.get(HandlerTypes_js_1.HandlerType.MACRO).lookup(cs)) {
            throw new TexError_js_1.default('AlreadyDefined', '%1 is already defined', '\\' + cs);
        }
        var handler = handlers.retrieve(NewcommandUtil_js_1.NewcommandTables.NEW_COMMAND);
        handler.add(cs, new Token_js_1.Macro(cs, exports.MathtoolsMethods.Aboxed, [box, star]));
    ...
    PairedDelimiters: function (parser, name, open, close, body, n, pre, post) {
        ...
        var _a = __read(star
            ...
            : size
                ? [size + 'l', size + 'r', '']
                : ['', '', ''], 3), left = _a[0], right = _a[1], after = _a[2];
        ...
        if (n) {
            ...
            pre = ParseUtil_js_1.ParseUtil.substituteArgs(parser, args, pre);
            body = ParseUtil_js_1.ParseUtil.substituteArgs(parser, args, body);
            post = ParseUtil_js_1.ParseUtil.substituteArgs(parser, args, post);
        }
        ...
        ].reduce(function (s, part) { return ParseUtil_js_1.ParseUtil.addArgs(parser, s, part); }, '');
        parser.i = 0;
    ...
    DeclarePairedDelimiter: function (parser, name) {
        ...
        MathtoolsUtil_js_1.MathtoolsUtil.addPairedDelims(parser, cs, [open, close]);
        parser.Push(parser.itemFactory.create('null'));
    },
    DeclarePairedDelimiterX: function (parser, name) {
        ...
        MathtoolsUtil_js_1.MathtoolsUtil.addPairedDelims(parser, cs, [open, close, body, n]);
        parser.Push(parser.itemFactory.create('null'));
    },
    DeclarePairedDelimiterXPP: function (parser, name) {
        ...
        var body = parser.GetArgument(name);
        MathtoolsUtil_js_1.MathtoolsUtil.addPairedDelims(parser, cs, [
            open,
            close,
            body,
            n,
    ...
    CenterColon: function (parser, _name, center, force, thin) {
        ...
        if (center && (options['centercolon'] || force)) {
            ...
            mml = parser.create('node', 'mpadded', [mml], __assign({ voffset: dy, height: "+".concat(dy), depth: "-".concat(dy) }, (thin
        ...
        if (options['use-unicode'] && unicode) {
            ...
            tex =
                '\\mathrel{' +
                    ...
                    '}';
            parser.string = ParseUtil_js_1.ParseUtil.addArgs(parser, tex, parser.string.substring(parser.i));
            parser.i = 0;
        }
    },
    ...
    SetOptions: function (parser, name) {
        ...
        var keys = ParseUtil_js_1.ParseUtil.keyvalOptions(args, allowed, true);
        try {
            for (var _b = __values(Object.keys(keys)), _c = _b.next(); !_c.done; _c = _b.next()) {
                var id = _c.value;
                if (id === 'legacycolonsymbols' && options[id] !== keys[id]) {
                    if (options[id]) {
                        parser.configuration.handlers.remove(exports.LEGACYCONFIG, {});
                    }
                    else {
                        parser.configuration.handlers.add(exports.LEGACYCONFIG, {}, exports.LEGACYPRIORITY);
                    }
                }
                options[id] = keys[id];
            }
