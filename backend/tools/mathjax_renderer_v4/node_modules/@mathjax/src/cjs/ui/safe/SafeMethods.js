...
exports.SafeMethods = {
    ...
    filterClassList: function (safe, list) {
        ...
        return classes
            .map(function (name) { return _this.filterClass(safe, name) || ''; })
            .join(' ')
            .trim()
            .replace(/\s\s+/g, '');
    },
    ...
    filterStyleLength: function (safe, style, value) {
        ...
        var _a = __read(Array.isArray(lengths)
            ? lengths
            : [-safe.options.lengthMax, safe.options.lengthMax], 2), m = _a[0], M = _a[1];
        return m <= em && em <= M
            ? value
    ...
    filterSizeMultiplier: function (safe, size) {
        var _a = __read(safe.options.scriptsizemultiplierRange || [
            -Infinity,
            Infinity,
        ], 2), m = _a[0], M = _a[1];
