...
def test_router_auto_order(monkeypatch):
    ...
    monkeypatch.setenv("VISION_OCR_ENABLED_PROVIDERS", "pix2txt,openai")
    monkeypatch.setenv("VISION_OCR_FALLBACK_ORDER", "pix2txt,openai")
    cfg = VisionRoutingConfig.from_env()
    assert build_provider_plan("auto", cfg) == ["pix2txt", "openai"]
