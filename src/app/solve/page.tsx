...
export default function DashboardPage() {
    ...
    const profileCompletion = useMemo(() => {
        const fields = [
            { label: "Country", filled: Boolean(userProfile?.profile_country?.trim()) },
            { label: "Province/Territory", filled: Boolean(userProfile?.profile_province_state?.trim()) },
            { label: "Grade Level", filled: Boolean(userProfile?.grade_level?.trim()) },
        ];
    ...
    const shouldShowPersonalizationCard = profileCompletion.percentage < 100;
    const whatsappLinked = Boolean(userProfile?.whatsapp_linked);
    const whatsappConnected = whatsappLinked && userProfile?.whatsapp_enabled !== false;
    const whatsappLockActive = Boolean(userProfile?.whatsapp_lock_active);
    const dismissIdentity = String(readyWallet?.user_id || userProfile?.email || "anon");
    const [personalizationTipDismissed, setPersonalizationTipDismissed] = useState(false);
    const [whatsappTipDismissed, setWhatsappTipDismissed] = useState(false);
    const shouldShowTopTips = shouldShowPersonalizationCard;
    const showPersonalizationTip = shouldShowTopTips && !personalizationTipDismissed;
    const showWhatsappTip = shouldShowTopTips && !whatsappTipDismissed;

    useEffect(() => {
        if (typeof window === "undefined") return;
        const personalizeKey = `uask.solve.tip.personalize.dismissed.${dismissIdentity}`;
        const whatsappKey = `uask.solve.tip.whatsapp.dismissed.${dismissIdentity}`;
        setPersonalizationTipDismissed(localStorage.getItem(personalizeKey) === "1");
        setWhatsappTipDismissed(localStorage.getItem(whatsappKey) === "1");
    }, [dismissIdentity]);

    const dismissPersonalizationTip = () => {
        setPersonalizationTipDismissed(true);
        if (typeof window !== "undefined") {
            localStorage.setItem(`uask.solve.tip.personalize.dismissed.${dismissIdentity}`, "1");
        }
    };
    const dismissWhatsappTip = () => {
        setWhatsappTipDismissed(true);
        if (typeof window !== "undefined") {
            localStorage.setItem(`uask.solve.tip.whatsapp.dismissed.${dismissIdentity}`, "1");
        }
    };
    ...
    const handleSolve = async (textOverride?: string, featureOverrides?: Record<string, unknown>) => {
        ...
        const isSnapOcrSolve = Boolean(featureOverrides?.ocr_used) || activeTab === "snap";
        const validationError = validateMathQuery(textToSolve);
        const canBypassValidationForSnap =
            isSnapOcrSolve &&
            validationError !== INPUT_ERROR_EMPTY &&
            validationError !== INPUT_ERROR_TOO_SHORT;
        const canBypassNotMathForSnap = isSnapOcrSolve && validationError === INPUT_ERROR_NOT_MATH;
        const isOverridableError = validationError === INPUT_ERROR_BLOCKED || validationError === INPUT_ERROR_NOT_MATH;

        if (validationError) {
            if (canBypassValidationForSnap || canBypassNotMathForSnap) {
                setInputError(null);
            } else if (isOverridableError) {
    ...
    return (
        <div dir={uiDirection} className="solve-ui bg-background-light dark:bg-background-dark min-h-screen text-slate-900 dark:text-slate-100 font-display transition-colors duration-200">
            ...
            <main className="max-w-6xl mx-auto px-4 py-5 md:py-7">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 [direction:ltr]">
                    {/* Main Interaction Area */}
                    <div className="lg:col-span-8 space-y-6" dir={uiDirection}>
                        {showPersonalizationTip && (
                            <div className="rounded-xl border border-amber-300/90 bg-amber-50/60 dark:bg-amber-950/20 p-5 relative">
                                <button
                                    onClick={dismissPersonalizationTip}
                                    className="absolute right-3 top-3 text-slate-500 hover:text-slate-800 dark:hover:text-slate-100"
                                    aria-label="Close personalization tip"
                                >
                                    <span className="material-symbols-outlined text-[18px]">close</span>
                                </button>
                                <div className="flex items-start gap-3 pr-6">
                                    <span className="material-symbols-outlined text-amber-500 mt-0.5">auto_fix_high</span>
                                    <div className="flex-1">
                                        <h3 className="text-base font-bold text-slate-900 dark:text-white">Personalize your responses</h3>
                                        <p className="mt-1 text-sm text-slate-600 dark:text-slate-300">
                                            Complete your profile once and we&apos;ll tailor answers to your level and location.
                                        </p>
                                        <p className="mt-2 text-xs text-slate-500">
                                            {profileCompletion.filledCount} of {profileCompletion.total} fields complete.
                                            {profileCompletion.missing.length > 0 ? ` Missing: ${profileCompletion.missing.slice(0, 3).join(", ")}${profileCompletion.missing.length > 3 ? "..." : ""}` : ""}
                                        </p>
                                        <button
                                            onClick={() => router.push("/profile")}
                                            className="mt-3 inline-flex items-center gap-2 rounded-lg bg-amber-500 px-3 py-2 text-xs font-bold text-white hover:bg-amber-600 transition-colors"
                                        >
                                            Complete Profile
                        ...
                        {showWhatsappTip && (
                            <div className={`rounded-xl border p-5 relative ${whatsappConnected ? "border-emerald-300 bg-emerald-50/70 dark:bg-emerald-950/20" : "border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900"}`}>
                                <button
                                    onClick={dismissWhatsappTip}
                                    className="absolute right-3 top-3 text-slate-500 hover:text-slate-800 dark:hover:text-slate-100"
                                ...
                                <div className="flex items-start gap-3 pr-6">
                                    <span className={`material-symbols-outlined mt-0.5 ${whatsappConnected ? "text-emerald-600" : "text-slate-500"}`}>chat</span>
                                    <div className="flex-1">
                                        <h3 className="text-base font-bold text-slate-900 dark:text-white">WhatsApp Integration</h3>
                                        {whatsappConnected ? (
                                            <>
                                                <p className="mt-1 text-sm text-emerald-700 dark:text-emerald-300 font-semibold">
                                                    WhatsApp already connected.
                                                </p>
                                                {whatsappLockActive && (
                                                    <p className="mt-1 text-xs text-amber-600 dark:text-amber-300">
                                                        WhatsApp is temporarily locked due to safety limits. You can manage this in profile.
                                                    </p>
                                                )}
                                                <button
                                                    onClick={() => router.push("/profile")}
                                                    className="mt-3 inline-flex items-center gap-2 rounded-lg border border-emerald-300 px-3 py-2 text-xs font-semibold text-emerald-700 dark:text-emerald-300 hover:bg-emerald-100/60 dark:hover:bg-emerald-900/30 transition-colors"
                                                >
                                                    Manage WhatsApp
                                                </button>
                                            </>
                                            ...
                                            <>
                                                <p className="mt-1 text-sm text-slate-600 dark:text-slate-300">
                                                    Connect WhatsApp to receive answers there and continue solving from your phone.
                                                </p>
                                                <button
                                                    onClick={() => router.push("/profile")}
                                                    className="mt-3 inline-flex items-center gap-2 rounded-lg bg-emerald-600 px-3 py-2 text-xs font-bold text-white hover:bg-emerald-700 transition-colors"
                                                >
                                                    Connect WhatsApp
                                                    <span className="material-symbols-outlined text-sm">arrow_forward</span>
                                                </button>
                ...
                {/* Sidebar */}
                <div className="lg:col-span-4 space-y-6" dir={uiDirection}>
