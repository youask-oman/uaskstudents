...
export default function AdminSubscriptionsPage() {
    ...
    const fetchPlansAndPrompts = async () => {
        try {
            ...
            const [plansRes, promptsRes] = await Promise.all([
            ...
            <header className="mb-8 flex justify-between items-center">
                ...
                <button
                    onClick={() => setSelectedPlan({
                        ...
                        is_active: false,
                        features: {},
                        multipliers: { "text_concise": 1, "text_detailed": 2 }
                    })}
                    ...
                    <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl">
                        ...
                        <div className="p-6 space-y-6">
                            ...
                            <div className="space-y-4 pt-4 border-t border-slate-800">
                                <h3 className="text-white font-bold">Linked Prompts</h3>
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="space-y-2">
                                        <label className="text-xs text-slate-400 uppercase font-bold">System Prompt</label>
                                        <select
                                            value={selectedPlan.system_prompt_template_id || ""}
                                            onChange={(e) => setSelectedPlan({ ...selectedPlan, system_prompt_template_id: e.target.value ? parseInt(e.target.value) : null })}
                                            className="w-full bg-slate-950 border border-slate-800 rounded-lg p-2.5 text-white focus:border-admin-primary outline-none"
                                        >
                                    ...
                                    <div className="space-y-2">
                                        <label className="text-xs text-slate-400 uppercase font-bold">Schema / Format Prompt</label>
                                        <select
                                            value={selectedPlan.schema_prompt_template_id || ""}
                                            onChange={(e) => setSelectedPlan({ ...selectedPlan, schema_prompt_template_id: e.target.value ? parseInt(e.target.value) : null })}
                                            className="w-full bg-slate-950 border border-slate-800 rounded-lg p-2.5 text-white focus:border-admin-primary outline-none"
                                        >
                                            <option value="">-- No linked prompt --</option>
                                            {prompts.map(p => (
                            ...
                            <div className="flex items-center gap-3 pt-4">
                                ...
                                />
                                <label htmlFor="isActive" className="text-white font-medium">Active Plan (Visible to users)</label>
                            </div>
                        </div>
