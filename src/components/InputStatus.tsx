...
import { MAX_INPUT_CHARS, MAX_INPUT_TOKENS } from '@/lib/tokenBudget';
...
export default function InputStatus({
    ...
    const tokenStatus = getTokenStatus(tokenEstimate, MAX_INPUT_TOKENS);

    // Determine colors based on status
    const getCharColor = () => {
        const pct = (charCount / MAX_INPUT_CHARS) * 100;
        ...
        <div className="flex flex-wrap items-center justify-between gap-2 mt-2 text-xs">
            ...
            <div className="flex items-center gap-4">
                ...
                <span className={getCharColor()}>
                    ...
                    <span className="opacity-60">/{MAX_INPUT_CHARS}</span>
                </span>

                {/* Token estimate */}
                <span className={getTokenColor()}>
                    Tokens: <span className="font-mono">{tokenEstimate.tokens}</span>
                    <span className="opacity-60">/{MAX_INPUT_TOKENS}</span>
